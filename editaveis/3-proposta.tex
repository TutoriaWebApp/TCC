...
\subsubsection[XP]{XP}
A metodologia XP é uma metodologia ágil de desenvolvimento de software que foca na aplicação de técnicas de programação, comunicação clara, \textit{feedback} constante e no trabalho em equipe \cite{Warden2003}.
Segundo \citeonline{BeckAndres2004} o que distingue o XP de outras metodologias são os seguintes pontos:
...
\begin{itemize}
    ...
    Dois desenvolvedores trabalham em conjunto em um único computador. Um escreve o código  enquanto o outro revisa, sugere melhorias e pensa estrategicamente. O papel foi intercalado entre os desenvolvedores, e com uma equipe de dois desenvolvedores, elimina-se a necessidade de uma etapa formal de revisão de código, pois o desenvolvimento conjunto permite comunicação constante, resolução imediata de dúvidas e tomada de decisões em conjunto.
...
Segundo \citeonline{Anderson2010} as práticas do \textit{Kanban} são:

\begin{itemize}
  \item \textbf{Visualizar:} Uma boa visualização para colaboração efetiva e para identificar possíveis melhorias. Visualizar este trabalho e o fluxo dele beneficia a transparência para absorver e processar muita informações num curto espaço de tempo.
  
  \item \textbf{Limite de Trabalho em Progresso:} Para equilibrar a utilização e garantir o fluxo de trabalho. Sistemas eficazes se concentram mais no fluxo de trabalho. 
  
  \item \textbf{Gerenciamento de Fluxo:} O objetivo é concluir o trabalho da forma mais contínua e previsível possível, assim como manter um ritmo saudável para os participantes. 
\end{itemize}
